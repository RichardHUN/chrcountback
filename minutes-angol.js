let GUI=1;
	...
	english.onclick = function(){
		window.open("./minutes-angol.html", "_self");
	}
	magyar.onclick = function(){
		window.open("./minutes.html", "_self");
	}
		function frissit(){
		...
		if(most.getMonth() == 11 && most.getDate() >= 24){
			maradek.day = 365*24*60 + newYear*24*60*60 - most.getMinutes();
		}

		switch(angol){
			case 0:
				document.getElementById("write").innerHTML = "Karácsonyig " + maradek.day + " perc van hátra hátra.";
				if(GUI%2!=0){
			...
			case 1:
				if(maradek.day>1)
					document.getElementById("write").innerHTML = "There are " + maradek.day + " minutes until Christmas.";
				if(maradek.day==1)
					document.getElementById("write").innerHTML = "There is " + maradek.day + " minute until Christmas.";
				if(GUI%2!=0){
